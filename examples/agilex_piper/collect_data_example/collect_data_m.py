...
class DataFileManager:
    ...
    def init_keys(self, observation, camera_keys) -> None:
        ...
        if self.leader_num == 1:
            ...
            with open("./replay2_test.json") as file:
                self.template_dict = json.load(file)
    ...
    def save_data(self) -> None:
        ...
        global mark_time
        if mark_time != 0:
            self.template_dict["data"]["/put_index"][0]["data"][0] = mark_time
            mark_time = 0
        save_bson(
...
class KeyHandler:
    ...
    def handle_keypress(self, key: keyboard.Key) -> None:
        ...
        """
        # 根据按键事件触发状态机的状态
        global mark_time_bool, mark_time
...
class DataCollecter:
    ...
    def Collect(self) -> None:
        ...
        while self.state == "Collect" and self.current_frame < self.max_frames:
            ...
            global mark_time_bool, mark_time
            if mark_time_bool is True:
                mark_time = self.current_frame
                mark_time_bool = False
            print("current: ", self.current_frame, "put_time:", mark_time)
            with self.lock:
